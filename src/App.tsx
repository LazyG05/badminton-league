...
const Icons = {
  ...
  <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    <path
      ...
      strokeWidth={2}
      d="M3 7l1 13h16l1-13M5 7l1-3h12l1 3M9 11v6m6-6v6"
    />
  </svg>
),
...
<a
  ...
  className="
    w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all font-medium border
...
// ========================= AttendanceCalendar =========================
function AttendanceCalendar({ players, attendance }: { players: Player[]; attendance: Record<string, string[]> }) {
  ...
  const playerStats = useMemo(() => {
    ...
    const attended = trainingDaysInMonth.filter(d => (attendance[d.date] ?? []).includes(selectedPlayer)).length;
  ...
  const avgAttendance = useMemo(() => {
    ...
    const total = trainingDaysInMonth.reduce((sum, d) => sum + (attendance[d.date]?.length ?? 0), 0);
  ...
  const totalCells = gridOffset + daysInMonth.length;
            ...
            if (selectedPlayer === "__all__") {
              ...
              return (
                <button
                  ...
                  className={`flex flex-col items-center justify-center rounded-lg py-1 min-h-[44px] transition-colors ${baseRing} ${
                    isExpanded ? "bg-[#f0fdf4]" : hasSession ? "bg-slate-50 hover:bg-slate-100" : "bg-transparent"
              ...
              return (
                <div
                  key={date}
                  className={`flex flex-col items-center justify-center rounded-lg py-1 min-h-[44px] ${baseRing} ${hasSession ? "bg-slate-50" : "bg-transparent"}`}
                >
                  <span className={`text-xs font-medium leading-none ${isToday ? "text-[#84cc16] font-bold" : "text-slate-600"}`}>{day}</span>
