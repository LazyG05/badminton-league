 ...
 *  - Admin view: add players, create/edit results
...
const weekday = (dstr: string) => new Date(dstr + "T12:00:00").toLocaleDateString(undefined, { weekday: "long" });
...
function makePairsForRound(ids: string[], seenTeammates: Set<string>): Pair[] {
  ...
  function backtrack(rem: string[], cur: Pair[]): Pair[] {
    ...
    for (let i=0;i<rest.length;i++) {
      const cand = rest[i]; if (seenTeammates.has(key(first, cand))) continue;
      const next = backtrack(rest.filter((_,idx)=>idx!==i), [...cur, [first, cand]] as Pair[]);
...
function useLeague() {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(ref, async (snap) => {
      ...
      else { await setDoc(ref, { players: [], matches: [], createdAt: serverTimestamp(), updatedAt: serverTimestamp() }); }
    });
    return () => unsub();
  }, []);
...
// ========================= Components =========================
function Header({ title, role, setPlayer, setAdmin }: { title?: string; role: "player"|"admin"; setPlayer: () => void; setAdmin: () => void; }) {
  const isAdmin = role === "admin";
  return (
    <header className="mb-4 flex flex-col gap-2 sm:mb-6 sm:flex-row sm:items-center sm:justify-between">
      <h1 className="text-xl font-bold sm:text-2xl">🏸 {title || "Bia-Tollas League"} – doubles, individual standings</h1>
      <div className="flex gap-2">
        <div className="rounded-xl border bg-white p-1">
          <button type="button" onClick={setPlayer} className={`${btnBase} ${!isAdmin ? 'bg-indigo-600 text-white' : 'bg-white text-gray-900'} px-3 py-1`}>Player</button>
          <button type="button" onClick={setAdmin}  className={`${btnBase} ${isAdmin ? 'bg-indigo-600 text-white' : 'bg-white text-gray-900'} px-3 py-1`}>Admin</button>
        </div>
      </div>
    </header>
  );
    ...
    <div className={card}>
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">{value} / {weekday(value)}</p>
        </div>
  ...
  return (
    <div className={card}>
      ...
      <div className="flex w-full flex-col gap-2 sm:flex-row">
        <input className={input} placeholder="Player name" value={name} onChange={(e)=> setName(e.target.value)} onKeyDown={(e)=>{ if(e.key==='Enter' && !disabled){ onAdd(name); setName(''); }}} disabled={!!disabled} />
        <button className={btnPrimary} onClick={()=>{ if(!disabled){ onAdd(name); setName(''); }}} disabled={!!disabled}>Add</button>
      </div>
      {players.length>0 && (
        <ul className="mt-3 divide-y text-sm">
          {players.map(p=> (
            <li key={p.id} className="flex items-center justify-between py-2">
              ...
              <button className={btnDanger} onClick={()=> onRemove(p.id)} disabled={!!disabled}>remove</button>
            </li>
          ))}
        </ul>
      )}
...
function DnDPairs({ players, freeIds, seenTeammates, onCreate, disabled }: { players: Player[]; freeIds: string[]; seenTeammates: Set<string>; onCreate: (a: Pair, b: Pair) => void; disabled?: boolean; }) {
  const [pool, setPool] = useState<string[]>(freeIds);
  const [teamA, setTeamA] = useState<string[]>([]);
  const [teamB, setTeamB] = useState<string[]>([]);
  useEffect(()=>{ setPool(freeIds); setTeamA([]); setTeamB([]); }, [freeIds.join(",")]);
  ...
  return (
    <div className={card}>
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        <button className={btnPrimary} disabled={!canCreate} onClick={()=>{ onCreate([teamA[0], teamA[1]], [teamB[0], teamB[1]]); setTeamA([]); setTeamB([]); }}>Add match</button>
        <button className={btnSecondary} onClick={()=>{ setPool(freeIds); setTeamA([]); setTeamB([]); }} disabled={!!disabled}>Reset</button>
        <span className="text-xs text-gray-500">Tip: 8 players → 2 matches, 12 → 3, etc. Add multiple matches in the same date.</span>
      </div>
    </div>
  );
}

function MatchesAdmin({ matches, nameOf, onPick, onClear }: { matches: Match[]; nameOf: (id: string)=>string; onPick: (id:string, w: 'A'|'B')=>void; onClear:(id:string)=>void; }) {
  return (
    <div className={card}>
      <ShuttleBg />
          ...
          {matches.map(m=> (
            <li key={m.id} className="flex items-center justify-between rounded-xl border p-3">
              ...
              <div className="flex items-center gap-2 text-sm">
                <button className={btnSecondary} onClick={()=> onPick(m.id, 'A')}>Team A</button>
                <button className={btnSecondary} onClick={()=> onPick(m.id, 'B')}>Team B</button>
                <button className={btnSecondary} onClick={()=> onClear(m.id)}>clear</button>
              </div>
            </li>
          ))}
        </ul>
...
function MatchesPlayer({ grouped, nameOf }: { grouped: {date: string; matches: Match[]}[]; nameOf: (id:string)=>string; }) {
  return (
    <div className={card}>
      <ShuttleBg />
      ...
      {grouped.length===0 ? (
        ...
        <div className="space-y-4">
          {grouped.map(g => (
            <div key={g.date}>
              <div className="mb-1 text-sm text-gray-600">{g.date} / {weekday(g.date)}</div>
              <ul className="divide-y">
                {g.matches.map(m => (
                  <li key={m.id} className="py-2 text-sm">
                    ...
                    {m.winner ? (
                      <span className="ml-2">– Winner: <b>{m.winner==='A' ? `${nameOf(m.teamA[0])} & ${nameOf(m.teamA[1])}` : `${nameOf(m.teamB[0])} & ${nameOf(m.teamB[1])}`}</b> 🏆</span>
                    ) : (
...
export default function App() {
  ...
  const isAdmin = role==='admin';
  const setPlayer = () => { localStorage.setItem('bia_role','player'); setRole('player'); };
  const setAdmin  = () => { localStorage.setItem('bia_role','admin');  setRole('admin'); };
  ...
  const matchesForDate = useMemo(()=> league.matches.filter(m => m.date===date), [league.matches, date]);
  ...
  const grouped = useMemo(()=>{
    ...
    league.matches.forEach(m => { (by[m.date] ||= []).push(m); });
    return Object.entries(by).sort(([a],[b])=> a.localeCompare(b)).map(([d, arr]) => ({ date: d, matches: arr }));
  }, [league.matches]);
  ...
  const clearWinner = (id: string) => { if(!isAdmin) return; write({ matches: league.matches.map(m=> m.id===id? { ...m, winner: undefined } : m) }); };
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900">
      <div className="mx-auto max-w-5xl p-4 sm:p-6">
        <Header title={league.title || 'Bia-Tollas'} role={role} setPlayer={setPlayer} setAdmin={setAdmin} />
            ...
            <div className="space-y-4">
              ...
              <div className={card}>
                ...
                <ul className="list-disc pl-5 text-sm text-gray-700 space-y-1">
                  <li>Admin creates pairings at the end of each session date.</li>
                  <li>Winners are recorded per match; standings are individual.</li>
